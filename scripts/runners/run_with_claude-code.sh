...
%s' "Continue the continuous-code-auditor audit. This is a non-interactive scheduled execution — no conversational memory is assumed; reconstruct everything from the workspace on disk as SKILL.md instructs. (If the continuous-code-auditor skill did not auto-activate for this message, that's a configuration problem — stop and report it rather than improvising an audit process.)" "$(auditor_context_block)"
}
